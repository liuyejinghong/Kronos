...
> 更新时间：2026-05-06 | 当前版本：0.4.1 | 下一版本：0.4.2

## 一句话判断

v0.4.1 没有新增运行时能力，而是完成了一轮基于用户画像的 Docker 首次体验评测。结论是：v0.4.0 的 Docker 安装链路合格，但首次研究体验仍有信任断点，尤其是 Agent 文案事实错误、`report latest` 摘要不足、7 天样本和 90 天文案冲突、Docker 路径误导、quickstart/策略配置粒度错配。
...
| Docker | 已完成 | `docker compose up` 可跑通 quickstart 主路径；多画像评测已记录下一轮体验修复项 |
...
| 优先级 | 问题 | 证据 |
|---|---|---|
| P0 | Agent 文案出现 `{n}` 未替换、7 天样本误写 90 天、1 个策略误写 12 个旧策略 | `docs/DOCKER_PERSONA_UX_EVALUATION_20260506.md` |
| P0 | `report latest` 只给内部流程摘要，不能形成交易用户第一屏判断 | `docs/DOCKER_PERSONA_UX_EVALUATION_20260506.md` |
| P0 | 自动研究报告中 7 天样本与“90 天复验已完成”冲突 | `docs/DOCKER_PERSONA_UX_EVALUATION_20260506.md` |
| P1 | Docker 用户容易混用宿主机 `~/.kronos` 和容器 `/root/.kronos` 路径 | `docs/DOCKER_PERSONA_UX_EVALUATION_20260506.md` |
| P1 | quickstart/Agent 使用 1m，TOML 默认 15m，用户难以确认自己验证的是哪套 R-breaker | `docs/DOCKER_PERSONA_UX_EVALUATION_20260506.md` |

## 当前推荐顺序

1. 优先修复 v0.4.1 评测暴露的 P0 体验问题：Agent 事实错误、`report latest` 第一屏、7 天/90 天文案冲突。
2. 修复 Docker 策略路径和 1m/15m 粒度解释，让 L2/L3 用户能沿同一条策略资产路径继续。
3. 再进入 AI 自然语言策略创建，但输出必须先落到 TOML 并经过同一条 smoke-test/register 闸门。
4. 后续做历史重放和市场状态分段评估，让用户能理解“为什么这段表现好/差”。
