...
```
用户描述 → LLM 生成 TOML → 用户确认 → 烟雾测试（3天数据验证逻辑正确性）
→ 写入 ~/.kronos/strategies/ → Agent 启动时自动加载 → 出现在策略池中
```

烟雾测试是必须步骤：用最近 3 天数据快速跑一遍，逐笔展示入场/出场逻辑是否符合用户描述。通过后才写入策略文件。如果逻辑不对，用户可以选择"修正"——描述问题，LLM 重新生成。
...
| 阶段 | v0.3.0 | 说明 |
|------|--------|------|
| 定义策略 | ✅ | R-breaker + AI 自然语言创建 |
| 验证回测 | ✅ | 历史数据上跑回测 |
| 历史重放 | ✅ | 不需要 API Key，本地数据重演买卖过程 |
| 实时模拟盘 | ⏳ v0.4.0 | 需要 Binance 只读 API Key |
| 实盘执行 | ⏳ v0.5.0+ | 需要 Binance 交易 API Key + 风控配置 |
...
#### 阶段 3: 实时模拟盘（v0.4，需要 Binance 只读 API Key）
...
### Phase 4: 历史重放模式（v0.3.0）

不需要 API Key。用已有历史数据模拟实时交易过程，用户可以看到每分钟发生了什么。

| 任务 | 说明 | 优先级 |
|------|------|--------|
| 重放引擎 | 1m K 线驱动，逐分钟产生信号、模拟成交、计算持仓 | P0 |
| 重放可视化 | 终端逐分钟输出（信号→订单→成交→持仓变化），可暂停/加速 | P0 |
| 重放摘要 | 每日/全周期统计：交易笔数、胜率、盈亏分布 | P0 |

### Phase 5: 实时模拟盘（v0.4.0）

借鉴 AItrading 的 `OneMinuteExecutionEngine` 和 `EventBus` 架构。需要 Binance 只读 API Key。

| 任务 | 说明 | 优先级 |
|------|------|--------|
| 事件总线 | `bar_closed` / `order_filled` / `position_changed` 事件类型 | P1 |
| 模拟盘引擎 | 实时行情 + 虚拟订单匹配，执行优先级: reverse > SL > TP > exit > entry | P1 |
| 运行时状态持久化 | SQLite 存储 StrategyRuntime（trailing tier, TP1 标记, 冷却规则） | P1 |
| 信号引擎共享 | 同一份策略代码用于回测和模拟盘 | P1 |
| 交易所连接 | Binance WebSocket 实时行情 | P1 |
